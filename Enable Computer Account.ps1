...
# Code Template V 1.2
# URL: https://au2mator.com/enable-computer-account-active-directory-self-service-with-au2mator/
...
try {
    Get-ADComputer -Identity $c_Computer | Enable-ADAccount -Confirm:$false
...
if ($ErrorCount -eq 0) {
    $au2matorReturn = "Computer $c_Computer enabled"
    $AdditionalHTML = "<br>
        Computer " + (Get-ADComputer -identity $c_Computer).DisplayName + " has been enabled
        <br>
        "
    $Status = "COMPLETED"
}
else {
    $au2matorReturn = "failed to enable Computer $c_Computer, Error: $Error"
    $Status = "ERROR"
}
